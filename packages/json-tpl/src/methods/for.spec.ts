import { compile, createObjectScope, evaluate } from '../index.js'
...
describe('var', () => {
  ...
  it('should validate the "do" property', () => {
    expect(() => compile({ '@for': 3 })).toThrowError('Missing argument "do"')
  })
  ...
  it('should be able to execute methods as "do" property', () => {
    expect(
      evalStrict(
        {
          '@define': {
            sayHello: { '@fn': { '@concat': ['Hello ', { '@var': '$argv' }, '!'] } },
          },
          in: {
            '@for': ['Matthieu', 'Jean', 'Paul'],
            // Same as:
            // do: { '@sayHello': { '@var': '$value' } },
            do: { '@var': 'sayHello' },
          },
        },
        scope
      )
    ).toStrictEqual(['Hello Matthieu!', 'Hello Jean!', 'Hello Paul!'])
  })
