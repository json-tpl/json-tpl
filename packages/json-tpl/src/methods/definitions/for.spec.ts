...
const evalStrict = (template: Json, scope: Scope) =>
  evaluate(template, scope, { onError: thrower, executionLimit: 1e6 })
...
describe('var', () => {
  ...
  it('should allow generating custom arrays', () => {
    expect(evalStrict({ '@for': 4 }, scope)).toStrictEqual([null, null, null, null])
  ...
  it('should trigger execution limit', () => {
    expect(() => evalStrict({ '@for': 1e6, do: 1 }, scope)).toThrowError(`Execution limit reached`)
    expect(() => evalStrict({ '@for': 1e6 }, scope)).toThrowError(`Execution limit reached`)
  })
